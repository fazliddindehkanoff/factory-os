...
describe('HTTP API', () => {
  ...
  it('creates and lists a request for a tenant-assigned user, end to end', async () => {
    ...
    await db.update(schema.users).set({ holdingId: h.id }).where(eq(schema.users.id, userId));
    ...
    expect(list.body[0].requestNumber).toMatch(/^REQ-\d{4}-00001$/);
  });
  ...
  it('forbids creating a request without the requests.create permission', async () => {
    ...
    await db.update(schema.users).set({ holdingId: h.id }).where(eq(schema.users.id, login.body.user.id));
    await request(app)
      .post('/api/requests')
      .set('Authorization', `Bearer ${token}`)
