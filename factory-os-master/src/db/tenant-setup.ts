...
// dept_head → warehouse → procurement(if not in stock) → finance(≥5M) → director(≥30M) → owner(≥100M)
const STEP_SPECS: StepSpec[] = [
  { code: 'dept_head', order: 1, name: 'Рук. отдела' },
  { code: 'warehouse', order: 2, name: 'Проверка склада' },
  { code: 'procurement', order: 3, name: 'Снабжение', condition: { inStock: false } },
  { code: 'finance', order: 4, name: 'Финансы', threshold: 5_000_000 },
  { code: 'director', order: 5, name: 'Директор', threshold: 30_000_000 },
  { code: 'owner', order: 6, name: 'Учредитель', threshold: 100_000_000 },
];
