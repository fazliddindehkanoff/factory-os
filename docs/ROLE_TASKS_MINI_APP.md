...
## Ещё не завершённые role-модули

- **Procurement** уже реализован: очередь, справочник поставщиков, несколько КП,
  выбор поставщика и возврат на повторный поиск работают через канонический lifecycle.
- **Finance**: действие `mark_paid` (PIN) на карточке заявки работает, но отдельной
  очереди платежей, счетов и платёжных документов пока нет.
- **Accountant**: read-only финансовая видимость есть; загрузка и привязка
  платёжных документов остаётся future-функцией.
- Полный актуальный аудит пробелов: `docs/IMPLEMENTATION_GAPS_2026-08-06.md`.
