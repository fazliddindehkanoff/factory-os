...
1. **Точка входа:** колокол в header с unread-бейджем. BottomNav не трогаем, в «Меню» не прячем.
2. **Фильтр MVP:** «Все / Непрочитанные». Отдельного failed-таба нет — failed показываем **бейджем внутри общего списка**.
3. **Failed без retry** — принято для MVP. `POST /me/notifications/:id/retry` в этот sprint НЕ добавляем; retry UI/API — отдельная маленькая задача позже.
4. **Auto mark-read при открытии заявки — да.** По клику на уведомление сначала mark-read, затем переход в заявку. Если mark-read не прошёл — заявка всё равно открывается (никакого fake success; ошибку отметки просто проглатываем/логируем, переход не блокируем).

Финальный scope зафиксирован. Реализация — по отдельной команде «go».
